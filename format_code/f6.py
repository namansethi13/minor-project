...
class f1:
    ...
    def process_data(self):
        for file_name, column_names in self.file_data.items():
            ...
            selected_columns = list(range(3)) + list(range(3, len(df1.columns), 3))
        ...
        self.df = self.df[['Enrollment No.', 'Name'] + self.subjects_to_consider]
        self.df = self.df[(self.df[self.subjects_to_consider] != 0).all(axis=1)]
        ...
        for subject in self.subjects_to_consider:
            top_bottom_df = top_bottom_df._append(self.df.nlargest(10, subject))
            top_bottom_df = top_bottom_df._append(self.df.nsmallest(10, subject))
    ...
    def write_to_doc(self):
        ...
        header_lines = [
            ...
            'Programme: BCA   Class:II Semester - Sec-A 	        Shift: I 	 Batch: 2016-2019	',
            'Max. Marks: 100							            Session: Aug-Dec 2019'
        ]
        ...
        for i in self.subjects_to_consider:
            ...
            table.cell(0, 1).text = f'Subject Name: {self.all_subjects[i]}    Class:{self.classes[index]}'
            ...
            for j in range(10):
                ...
                table.cell(3+j, 1).text = f"{int(self.df.iloc[j+20*index, 0]):011d}"
                table.cell(3+j, 2).text = str(self.df.iloc[j+20*index, 1]).casefold().title()
                table.cell(3+j, 3).text = str(int(self.df.iloc[j+20*index, 2+index]))
                table.cell(3+j, 4).text = f"{int(self.df.iloc[j+10+20*index, 0]):011d}"
                table.cell(3+j, 5).text = str(self.df.iloc[j+10 +20*index, 1]).casefold().title()
                table.cell(3+j, 6).text = str(int(self.df.iloc[j+10 +20*index, 2+index]))
            
                #remove the first 20 rows and 4th column from the dataframe
...
file_data = {
    "sem2.xlsx": ["S.no","Enrollment No.","Name","B", '020102', '020104', '020106', '020108', '020110', '020136', '020172', '020174', '020176'],
    "sem3.xlsx": ["S.no","Enrollment No.","Name","A", '020202', '020204', '020206', '020208', '020210', '020236', '020272', '020274']
}
