...
class Format_2:
    ...
    # Assuming the column you want to work with is named 'YourColumnName'
        ...
        for i, column_name in enumerate(column_names):
            ...
            self.filtered_df.at[i, 'Faculty Name'] = str(
...
# Specify the range of cells to merge (indices are zero-based)
        merge_cells(table, start_row=len(self.filtered_df)-1, start_col=3, end_row=len(self.filtered_df)-1, end_col=5)  # Merging columns 7 to 9
        merge_cells(table, start_row=len(self.filtered_df)-1, start_col=6, end_row=len(self.filtered_df)-1, end_col=8)
        merge_cells(table, start_row=len(self.filtered_df)-1, start_col=9, end_row=len(self.filtered_df)-1, end_col=11)
        merge_cells(table, start_row=len(self.filtered_df), start_col=3, end_row=len(self.filtered_df), end_col=5)  # Merging columns 7 to 9
        merge_cells(table, start_row=len(self.filtered_df), start_col=6, end_row=len(self.filtered_df), end_col=8)
        merge_cells(table, start_row=len(self.filtered_df), start_col=9, end_row=len(self.filtered_df), end_col=11)
        
# Add the footer
        footer_lines = ["",
            "         Class Coordinator	                  Result Analysis Committee		  HOD, BBA (M)",
...
f1.read_from_filtered_excel("BCA",subject_codes)
f1.write_to_doc()