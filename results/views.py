...
def normalize(request):
    ...
    os.makedirs(os.path.join(os.path.dirname(
        __file__), "buffer_files"), exist_ok=True)
    ...
    try:
        ...
        subjects = Subject.objects.filter(
            course=course, semester=request.POST['semester'])
        ...
        if is_elective.upper() == "true".upper():
            ...
            E = ElectiveDf(csv_file, subject_name_mapping=subject_name_mapping, exclude_subject_dict=exclude_subject_dict,
                           credits_mapping=credits_mapping, is_elective=is_elective, elective_obj=json_data)
        ...
        try:
            processor = ResultProcessor(csv_file, f'{random_file_name}.xlsx', subject_name_mapping,
                                        exclude_subject_dict, footers_to_add, headers_to_add, credits_mapping)
            if is_elective.upper() == "true".upper():
        ...
        if is_saved:
            ...
            with open(os.path.join(os.path.dirname(__file__), "buffer_files", f"{random_file_name}.xlsx"), "rb") as excel:
                ...
                instance = Result.objects.create(
                    course=course, passout_year=request.POST['passing'], semester=request.POST['semester'], xlsx_file=file_object, result_json=result_json)

            file_path = os.path.join(os.path.dirname(
                __file__), "buffer_files", f"{random_file_name}.xlsx")
            os.remove(file_path)
...
def check_result(request):
    ...
    course, passing = json.loads(request.body)[
        'course'], json.loads(request.body)['passing']
...
def download_result(request, id):
    ...
    response = HttpResponse(
        result.xlsx_file, content_type='application/ms-excel')
...
def update_result(request):
    ...
    try:
        result = Result.objects.get(
            course=course, passout_year=passing, semester=semester)
...
def format1(request):
    if request.method == "GET":
        ...
        all_subjects = Subject.objects.filter(
            course=course_model, semester=semester)

        subject_code_name_mapping = {
    ...
    if request.method == "POST":
        ...
        for e in data:
            ...
            course = entry['course']
            course_model = Course.objects.get(id=course)
            result_json = Result.objects.get(
                course=course_model, passout_year=entry['passing'], semester=entry['semester']).result_json
            result_name = Result.objects.get(
                course=course_model, passout_year=entry['passing'], semester=entry['semester']).xlsx_file.name
            result_df = pd.read_json(result_json)
            print("result df from the view file \n", result_df)
            try:
                all_subjects_objects = Subject.objects.filter(
                    course=course_model, semester=entry['semester'])
            except Subject.DoesNotExist:
                return HttpResponse("no result found", status=404)
            semester = entry['semester']
            year = entry['passing']
            all_subjects.update(
                {subject.code: subject.subject for subject in all_subjects_objects})
        ...
        format1 = f1(file_data=file_data, all_subjects=all_subjects,
                     faculty_name=faculty_name, shift=shift, semester=semester, passing=year)
        file_name = format1.write_to_doc()
        file_path = os.path.join(os.path.dirname(
            __file__), "buffer_files", file_name)
...
def format2(request):
    if request.method == "POST":
        ...
        try:
            ...
            all_subjects = {
                subject.code: subject.subject for subject in all_subjects}
            xlsxfile_name = Result.objects.get(
                course=course, passout_year=passout_year, semester=semester).xlsx_file.name
            json_data = Result.objects.get(
                course=course, passout_year=passout_year, semester=semester).result_json
        ...
        format2 = Format_2(result_df, all_subjects, course, semester,
                           shift, section, batch, passout_year, faculty_name, month)
    ...
    if request.method == "GET":
        ...
        all_subjects = Subject.objects.filter(
            course=course.upper(), semester=semester)

        subject_teacher_mapping = {
            subject.code: "DR. ABC" for subject in all_subjects}
...
def format6(request):
    ...
    for i, key in enumerate(keysofdata):
        ...
        try:
            course = Course.objects.get(id=course)
            Resultobject = Result.objects.get(
                course=course, passout_year=data[key]['passing'], semester=semester)
            print(Resultobject)
    ...
    for i in range(len(all_courses)):
        ...
        common_letters = [
            letter for letter in common_letters if letter in all_courses[i]]

    common_letters_string = ''.join(common_letters)
    f6 = Format6(file_data, dict_of_all_subjects, faculty_name=faculty_name, shift=main_shift,
                 course=common_letters_string, month=month, admitted_years=admitted_years, all_semesters=all_semesters)
...
def format7(request):
    if request.method == "GET":
        ...
        subject_list = {
            subject.code: subject.subject for subject in all_subjects}
        practical_subject_list = {
            subject.code: subject.subject for subject in all_subjects if subject.is_practical}
    ...
    if request.method == "POST":
        ...
        semester = data['semester']
        shift = course.shift
        # adding this here because data is being passed to the format7 class and shift is being is accessed from data
        data['shift'] = shift
        passout_year = data['passing']
        faculty_name = data['faculty_name']
        ...
        all_subjects_dict = {
            subject.code: subject.subject for subject in all_subjects}
        non_university_subjects = {
            subject.code: subject.subject for subject in all_subjects if subject.is_not_university}
        format7 = Format7(resut_df, data, faculty_name,
                          all_subjects_dict, admitted, non_university_subjects)
    ...
    if request.method == "GET":
        ...
        if action == None:
            response = HttpResponse(json.dumps(
                "NONE Please provide a valid get argument: 'action=template': to fetch the blank csv file , 'action=fetch,course,passout': to fetch data of students of a course 'action=fetch_file,course,passout': to fetch data of students of a course in file format"), status=400)
            return response
        if action.lower() == "template":
            ...
            try:
                course = Course.objects.get(id=course)
                student_data = StudentData.objects.get(
                    course=course, passout_year=passout)
                students_info = student_data.students_info_json
                dropped_students = student_data.dropped_sudents_json
                response = HttpResponse(json.dumps({"student_info": json.loads(
                    students_info), "dropped_students": json.loads(dropped_students)}), content_type='application/json')
                return response
            ...
            try:
                ...
                response = HttpResponse(
                    csv_file, content_type='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    ...
    if request.method == "POST":
        ...
        try:
            ...
            student_data = StudentData.objects.create(
                course=course, passout_year=passout, students_info_json=student_data_json, dropped_sudents_json=dropped_students)
...
def format11(request):
    ...
    if request.method == "POST":
        ...
        for e in data:
            ...
            shift = course_model.shift
        practical_subjects = [
            subject.code for subject in all_subjects_objects if subject.is_practical]

        format11 = f11(file_data=file_data, all_subjects=all_subjects, faculty_name=faculty_name,
                       shift=shift, semester=semester, passing=year, practical_subjects=practical_subjects)
...
def format5(request):
    ...
    student_data = StudentData.objects.get(
        course=course, passout_year=reqdata['passout_year'])
...
def format13(reqeust):
    if reqeust.method == "GET":
        ...
        subject_list = {
            subject.code: subject.subject for subject in subject_list}
    ...
    all_subjects = Subject.objects.filter(course=course, semester=semester)
    practical_subjects = [
        subject.code for subject in all_subjects if subject.is_practical]
    ...
    df = result_df.iloc[:, [1, 2, subject_index,
                            subject_index+1, subject_index+2]]
    ...
    format13 = f13(request_data, shift, year, faculty_name, semester,
                   section=reqdata['section'], is_practical=is_practical)
