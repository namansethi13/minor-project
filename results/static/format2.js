...
async function format2(semester, course) { // this is to get all the subjects of the semester and course
    try {
        const response = await axios.get(`http://127.0.0.1:8000/results/format2/?semester=${semester}&course=${course}`, {
            headers: {
...
async function format2Post(subjectTeacherMapping) {
    ...
    try {
        const response = await axios.post('http://127.0.0.1:8000/results/format2/', data, {
            headers: {
                'Content-Type': 'application/json',
            },
